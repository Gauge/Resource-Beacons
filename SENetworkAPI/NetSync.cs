...
namespace SENetworkAPI
{
	...
	public abstract class NetSync
	{
		...
		/// <summary>
		/// Triggers when the fetch request is recieved and before the value is sent back
		/// this is so the value can be updated before responding to the fetch.
		/// </summary>
		public Action<ulong> FetchRequest;
	...
	public class NetSync<T> : NetSync
	{
		...
		private void SendValue(SyncType syncType = SyncType.Broadcast, ulong sendTo = ulong.MinValue)
		{
			...
			if (syncType != SyncType.Fetch)
			{
				...
				{
					prepData = MyAPIGateway.Utilities.SerializeToBinary(_value);
				}
			}
		...
		internal static void RouteMessage(SyncData pack, ulong sender)
		{
			try
			{
				...
				{
					property.FetchRequest?.Invoke(sender);
