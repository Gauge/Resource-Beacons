...
namespace ResourceBaseBlock
{
	...
	public class ResourceBaseNode : MyGameLogicComponent
	{
		...
		public override void Init(MyObjectBuilder_EntityBase objectBuilder)
		{
			...
			MyVisualScriptLogicProvider.ButtonPressedEntityName += ButtonPressed;
		...
		private void ButtonPressed(string name, int button, long playerId, long blockId)
		{
			...
			try
			{
				MyLog.Default.Info($"[Resource Bases] activated: {name}, id: {button}, block {blockId}");

				IMyEntity ent = MyAPIGateway.Entities.GetEntityById(blockId);
				IMyButtonPanel panel = ent as IMyButtonPanel;

				List<IMyPlayer> players = new List<IMyPlayer>();
				MyAPIGateway.Players.GetPlayers(players, (p) => { return p.IdentityId == playerId; });

				if (panel != null && players.Count > 0)
				{
					string buttonName = panel.GetButtonName(button);

					foreach (var r in Core.Config.Resources)
					{
						if (buttonName.Contains(r.PrimaryName))
						{
							Activate(r, players[0]);
							break;
						}
					}
				}
			...
			{
				Tools.Log(MyLogSeverity.Error, $"[Resource Bases] Failed to initialize {e}");
			}
		}
